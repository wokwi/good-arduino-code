...
In reality, you won't be using Charlieplexing in you just need to drive 2 LEDs. It doesn't make sense.
But I included this example to explain the basic logic behind Charlieplexing. Let's see how we can generalize this code so that it works for any number of pins:
...
TODO In the previous code example, we made the TODO

As the number of LEDs is only two, you were able to address them individually. The code is very simple and easy to understand but not modular.
You cannot expand the code easily for three pins (to drive 6 LEDs using charlieplexing), for example. Hence we put some logic so that the same goal is achieved but with a different code.

You can find the modified code and simulation output below 👇

```
/**
   Charlieplexed LED Matrix Scanning demo to drive two LEDs using two port pins.
...
In this case, you can drive 3 \* 2 = 6 LEDs. You can find the connection diagram below 👇
...
```
/**
   Charlieplexed LED Matrix Scanning demo for 3 port pins to control 6 LEDs.
...
**Since we made our code modular, you had to only** add the additional port pin in the `LEDS[]` array.

### Case 3: Driving six LEDs with three Arduino Pins-Charlieplexing

In this case, you can drive 4 \* 3 = 12 LEDs. The connection diagram is shown below 👇
...
```
/**
   Charlieplexed LED Matrix Scanning demo for four ports.
...
In this case, you can drive 5 \* 4 = 20 LEDs. You can have a look at the connection diagram below 👇
...
```
/**
   Charlieplexed LED Matrix Scanning demo. example code for 5 port pins
...
In this case, you can drive 6 \* 5 = 30 LEDs. You can have a look at the connection diagram below 👇
...
```
/**
   Charlieplexed LED Matrix Scanning demo. example code for 6 port pins
...
In this case, you can drive 7 \* 6 = 42 LEDs. You can have a look at the connection diagram below 👇
...
```
/**
   Charlieplexed LED Matrix Scanning demo. example code to drive 42 LEDs using 7 port pins
...
In this case, you can drive 8 \* 7 = 56 LEDs. You can find the connection diagram below 👇
...
You have 56 LEDs in this case:

<figure>
  <CharlieplexingMatrix size={8} speed={500} />
  <figcaption>
...
```
**
   Charlieplexed LED Matrix Scanning demo. example code to drive 56 LEDs using 8 port pins
...
By now, we know how much helpful the [Arduino playground](https://wokwi.com/) is to develop our code as well as tweak the existing code.
We strongly encourage you to experiment with the playground and come up with new patterns or even display some text.
...
In this case, you can drive 9 \* 8 = 72 LEDs. The connection diagram is left out intentionally.
Based on the above cases, deriving the connection diagram should be super easy by now.
If you have any questions, [drop us an email!](mailto:uri+charlieplexing@wokwi.com)

You have 72 LEDs in this case:

<figure>
  <CharlieplexingMatrix size={9} speed={500} />
  <figcaption>
...
```
/**
   Charlieplexed LED Matrix Scanning demo. example code for 9 port pins to control 72 LEDs
...
## A few thoughts about Charlieplexing
1.  Charlieplexing is best suited where there is a need to drive only a few LEDs at a time
2.  The brightness of the LED would go down or become uneven when we are driving many LEDs at the same time
3.  If the leakage current of the MCU is high, the LEDs might still be lit (poorly, but visible in the dark) when the pins are configured as input.
4.  Debugging the hardware and wiring gets clumsy as pins are more
5.  Coding is easier with external multiplexers or shift registers than this technique

---

Your comments and feedback are always welcome😊.
If this article was helpful, please vote up, share this article with others as well.
You can also link your projects you built using this article in the comments for sure 👍
